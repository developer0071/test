...
function handleAlreadySubmitted() {
  let attempts = +localStorage.getItem("listening3_cheat") || 0;
  attempts++;
  localStorage.setItem("listening3_cheat", attempts);
...
function checkAnswers() {
  if (localStorage.getItem("listening3_submitted")) {
  ...
  for (let i = 21; i <= 30; i++) {
    ...
    // Save
    localStorage.setItem("listening3_" + qid, userRaw);
  ...
  localStorage.setItem("listening3_result", resultText);
...
window.addEventListener("load", () => {
  for (let i = 21; i <= 30; i++) {
    ...
    const saved = localStorage.getItem("listening3_" + qid);
    if (saved !== null && saved !== undefined) {
      ...
      if (localStorage.getItem("listening3_submitted") === "true") {
  ...
  const savedResult = localStorage.getItem("listening3_result");
...
function downloadPDF() {
  ...
  for (let i = 21; i <= 30; i++) {
    const qid = "q" + i;
    const user = localStorage.getItem("listening3_" + qid) || "(no answer)";
  ...
  y += 8;
  const result = localStorage.getItem("listening3_result") || "";
  doc.setFontSize(12);
...
function resetStats() {
  ...
  Object.keys(localStorage)
    .filter(k => k.startsWith("listening3_"))
    .forEach(k => localStorage.removeItem(k));

  // Also remove the cheat counter
  localStorage.removeItem("listening3_cheat");
